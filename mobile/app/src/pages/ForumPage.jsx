...
const RoleBadge = ({ author }) => {
  ...
  useEffect(() => {
    if (session?.user?.id) {
      supabase.from('profiles').select('role, is_pro, email, avatar_url, subscription_end_date, tier, subscription_status').eq('id', session.user.id).single().then(({ data }) => {
  ...
  return (
    <>
      ...
      <div onMouseMove={e => {
        ...
        {/* Floating Header */}
        <div style={{ position: 'sticky', top: 0, zIndex: 100, background: 'linear-gradient(to bottom, rgba(0,0,0,0.8), transparent)', padding: '24px 40px', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              <button onClick={goBack} style={{ color: '#fff', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', padding: '10px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: '0.3s' }} className="forum-glass-btn-icon"><ArrowLeft size={18} /></button>
              <h1 style={{ fontSize: '18px', fontWeight: '900', letterSpacing: '-0.5px', margin: 0, color: '#ccff00', textTransform: 'uppercase' }}>
                {view === 'categories' ? 'FORUM CULT //' : view === 'threads' ? currentCategory?.name : currentThread?.title?.substring(0, 30) + '...'}
              </h1>
              {view === 'categories' && <span style={{ padding: '4px 12px', background: 'rgba(168,255,62,0.1)', border: '1px solid rgba(168,255,62,0.2)', borderRadius: '99px', fontSize: '10px', fontWeight: '800', color: '#A8FF3E' }}>EARLY ACCESS</span>}
            </div>
            <div style={{ display: 'flex', gap: '12px' }}>
              {session && view === 'threads' && getCategoryAccess(currentCategory) === 'full' && (
                <button className="forum-glass-btn primary" onClick={() => { setNewCatId(currentCategory?.id || ''); setShowNewThread(true); }}>
                  <Plus size={16} /> New Thread
                </button>
              )}
              {session && <button onClick={() => navigate('/ai')} className="forum-glass-btn">Dashboard</button>}
            </div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 24px' }}>
            ...
            <div className="forum-fade" style={{ maxWidth: '1000px', margin: '0 auto', padding: '60px 0' }}>
              ...
              {loading ? (
                ...
                <div style={{ textAlign: 'center', padding: '100px 0', background: 'rgba(10,10,10,0.5)', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  {session && getCategoryAccess(currentCategory) === 'full' && <button className="forum-glass-btn primary" onClick={() => { setNewCatId(currentCategory?.id || ''); setShowNewThread(true); }} style={{ margin: '0 auto' }}><Plus size={16} /> Start Discussion</button>}
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  {(threads || []).map((t, i) => (
                    <div key={t.id} className="thread-row" onClick={() => openThread(t)} style={{ opacity: 0, animation: `forumFadeIn 0.5s cubic-bezier(0.16, 1, 0.3, 1) ${i * 0.05}s forwards` }}>
                      ...
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'center', zIndex: 2 }}>
                        ...
                        {isAdmin && (
                          <div style={{ display: 'flex', gap: '6px', marginLeft: '12px', borderLeft: '1px solid rgba(255,255,255,0.1)', paddingLeft: '12px' }} onClick={e => e.stopPropagation()}>
                            <button onClick={() => handleDeleteThread(t.id)} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', padding: '4px', opacity: 0.7 }}><X size={16} /></button>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          {view === 'thread' && currentThread && (
            <div className="forum-fade">
              ...
              {/* Reply Box */}
              {session && getCategoryAccess(currentCategory) === 'read-only' ? (
              ...
              ) : session && !currentThread.is_locked ? (
