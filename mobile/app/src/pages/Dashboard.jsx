...
import MobileCreditWidget from '../components/MobileCreditWidget';
...
import { getTabsForMode, getProTabsForMode, normalizeMode } from '../lib/promptModeRegistry';
import { Copy, Download, Zap, ChevronDown, ChevronUp, Share2, Globe, Lock, Flame, AlertCircle, X, Check, Crown, ArrowLeft, CheckCircle, FastForward } from 'lucide-react';
import ProExclusiveBadge from '../components/ProExclusiveBadge';
import jsPDF from 'jspdf';
import html2canvas from 'html2canvas';
import confetti from 'canvas-confetti';
import { timeAgo } from '../lib/utils';
import { canGenerate, logUsage, checkProStatus } from '../lib/pro';
import { getCredits, deductCredits, getActionCost, getResetCountdown } from '../lib/credits';
...
const Dashboard = ({ session }) => {
  ...
  // Usage & Pro state
  const [usageInfo, setUsageInfo] = useState({ allowed: true, isPro: false, used: 0, limit: 10 });
  const [showUpgradeModal, setShowUpgradeModal] = useState(false);
  const [creditInfo, setCreditInfo] = useState({ balance: 100, lastReset: new Date().toISOString() });
  ...
  useEffect(() => {
    ...
    if (params.get('upgrade') === 'success') {
      confetti({
        ...
        spread: 70,
        origin: { y: 0.6 },
        colors: ['#a3e635', '#6d28d9', '#ffffff']
      });
      showToast('Welcome to Prompt Quill Pro! 🚀');
    }
  }, [session.user.id, location.search]);

  // Update reset countdown every second - Match TopBar calculation
  useEffect(() => {
    if (!creditInfo?.lastReset) {
      return;
    }

    const updateCountdown = () => {
      try {
        const lastReset = new Date(creditInfo.lastReset);
        const nextResetTime = new Date(lastReset.getTime() + 24 * 60 * 60 * 1000);
        const now = new Date();
        const diffMs = nextResetTime - now;

        if (diffMs <= 0) {
          setResetCountdown({ hours: 0, minutes: 0, seconds: 0 });
          return;
        }
  ...
  const refreshUsage = async () => {
    try {
      const info = await canGenerate(session.user.id);
      setUsageInfo(info);

      // Also refresh profile specifically for is_pro sync
    ...
    try {
      ...
      // Check if user has 0 credits. If so, persistently show the upgrade modal
      const isProUser = usageInfo?.isPro || false;
      if (!isProUser && credits.balance <= 0) {
        console.log('[Dashboard] ❌ User has 0 credits on refresh/login. Showing modal.');
  ...
  const handleTogglePublic = async () => {
    ...
    console.log(`[handleTogglePublic] Session ID: ${currentSession.id}, User ID: ${session.user.id}`);
  ...
  const handleSessionSelect = async (id) => {
    ...
    if (versions?.length > 0 && versions[0].prompt_text) {
      try {
        ...
        const primaryPromptTab = getPrimaryPromptTabId(nextMode);
        setLoadedTabs(new Set([...Object.keys(parsed?.tabs || {}), primaryPromptTab]));
      } catch (e) {
  ...
  const handleVersionSelect = async (version) => {
    ...
    if (vData?.prompt_text) {
      try {
        ...
        setActiveTab(firstTab);
        const primaryPromptTab = getPrimaryPromptTabId(parsed?.mode || currentSession?.mode || 'GENERAL');
        setLoadedTabs(new Set([...Object.keys(parsed?.tabs || {}), primaryPromptTab]));
      } catch (e) {
  ...
  const handleGenerate = async (input, mode, metadata = {}) => {
    ...
    // Credit check
    const credits = await getCredits(session.user.id).catch(() => ({ balance: 100 }));
    setCreditInfo(credits);

    // Usage limit check first to get Pro status
    const usage = await canGenerate(session.user.id).catch(() => ({ allowed: true, isPro: false, used: 0, limit: 10 }));
    setUsageInfo(usage);

    // For non-Pro users: check credit and usage limits
    if (!usage.isPro) {
    ...
    // For Pro users: silently continue (they have unlimited credits)

    // Pro-only mode check — don't show upgrade for Pro users
    if (mode === 'STARTUP' && !usage.isPro) {
      setUsageInfo(usage);
      setShowUpgradeModal(true);
      return;
    }
  ...
  const proceedWithGenerationUsingTabs = async (enrichedInput, mode, metadata, tabsToUse) => {
    ...
    try {
      ...
      if (gptResult?.success === false) {
        ...
        } else if (gptResult.type === 'pro_required' || gptResult.error?.includes('Pro')) {
          setUsageInfo({ ...usageInfo, isPro: false });
          setShowUpgradeModal(true);
        } else {
      ...
      // Fetch latest credits to check if we need to limit tabs
      const currentCredits = await getCredits(session.user.id).catch(() => ({ balance: 100 }));
      const isLowCredits = !usageInfo?.isPro && currentCredits.balance > 0 && currentCredits.balance < 10;
      ...
      const INITIAL_GENERATED_TABS = isLowCredits
        ? new Set(['action_brief'])
        : new Set(['action_brief', 'steps', 'quick_wins']);
      setLoadedTabs(INITIAL_GENERATED_TABS);
      console.log(`[Dashboard] ✅ Initial tabs generated. Low credits mode: ${isLowCredits}`);
      ...
      // Deduct credits using dynamic cost based on output quality/length
      // For lazy-load (3 tabs), use the calculated cost from generateInitialTabs
      let costDeducted = gptResult._creditsForInitial || getActionCost('generate', gptResult);
      if (mode === 'CREATIVE' && !gptResult._initialTabsGeneration) {
        costDeducted = Math.ceil(costDeducted * 1.5);
        console.log(`[Creative Mode] Base cost: ${getActionCost('generate', gptResult)}, Adjusted cost: ${costDeducted}`);
      }

      // If low on credits, cap the cost to their remaining balance
      if (!usageInfo?.isPro && costDeducted > currentCredits.balance) {
        costDeducted = currentCredits.balance;
      }

      // Prevent double deduction if backend already deducted credits (like in STARTUP_LITE)
      if (gptResult.creditsUsed > 0) {
        console.log(`[Dashboard] 💳 Backend already deducted ${gptResult.creditsUsed} credits. Skipping frontend deduction.`);

        // Refresh credits display
        const updatedCredits = await getCredits(session.user.id);
        setCreditInfo(updatedCredits);

        // Show warning if credits are running low
        if (updatedCredits.balance <= 5 && updatedCredits.balance > 0) {
          showToast('⚠️ Only ' + updatedCredits.balance + ' credits left!');
        } else if (updatedCredits.balance === 0) {
          showToast('🚨 Out of credits! Top up to continue.');
        }
      } else {
        console.log(`[Dashboard] 💳 Deducting ${costDeducted} credits for lazy-load generation`);
        const deductResult = await deductCredits(session.user.id, costDeducted, gptResult._providerUsed || 'groq', `Generated: ${enrichedInput.substring(0, 50)} [${mode}]`);

        if (deductResult.success) {
          console.log(`✓ Deducted ${costDeducted} credits. Remaining: ${deductResult.remaining}`);
          // Refresh credits display
          const updatedCredits = await getCredits(session.user.id);
          setCreditInfo(updatedCredits);
          ...
          showToast('Error deducting credits: ' + deductResult.message);
        }
      }

      // Log usage
      logUsage(session.user.id, 'generate').catch(() => { });
      refreshUsage();
    } catch (error) {
...
`;
  ...
  const handleLoadTabOnDemand = async (tabId) => {
    ...
    const hasInitialContent =
      ...
      (result?.tabs?.[tabId] &&
        typeof result.tabs[tabId] === 'string' &&
        result.tabs[tabId].length > 10 &&
        !result.tabs[tabId].startsWith('['));
    ...
    // 💳 BEFORE API CALL: Check if user has enough credits (5 credits for on-demand tab)
    if (!usageInfo?.isPro) {
      const currentCredits = await getCredits(session.user.id);
      setCreditInfo(currentCredits);
      if (currentCredits.balance < 5) {
        console.log(`[Dashboard] ❌ Blocked tab generation: Insufficient credits (${currentCredits.balance} < 5)`);
        setShowUpgradeModal(true);
        return;
      }
    }
    ...
    try {
      ...
      if (tabContent && tabContent.success) {
        ...
        // 💳 Explicitly deduct 5 credits for on-demand tab generation
        const creditCost = 5;
        console.log(`[Dashboard] 💳 DEDUCTING ${creditCost} credits for tab: ${tabId}`);
        const deductResult = await deductCredits(session.user.id, creditCost, 'groq', `On-demand tab: ${tabId}`);

        if (deductResult.success) {
          console.log(`[Dashboard] ✅ DEDUCTED ${creditCost} credits. Remaining: ${deductResult.remaining}`);
          // Update UI with new balance
          const updated = await getCredits(session.user.id);
          setCreditInfo(updated);
          showToast(`✅ Loaded "${tabId.replace(/_/g, ' ')}" tab (5 credits used) • Balance: ${updated.balance}`);

          // Show warning if running low
          if (updated.balance <= 5 && updated.balance > 0) {
            showToast('⚠️ Only ' + updated.balance + ' credits left!');
          }
        } else {
          console.error(`[Dashboard] ❌ CREDIT DEDUCTION FAILED:`, deductResult.message);
          showToast(`⚠️ Tab loaded but credit deduction failed: ${deductResult.message}`);
        }
      } else {
...
`;
  ...
  const buildTabsFromResult = () => {
    ...
    // For STARTUP_LITE mode, all users have pro access
    const isStartupLite = result.mode === 'STARTUP_LITE';
    const isPro = isStartupLite ? true : (usageInfo?.isPro || false);
    ...
    const proTabs = getModeProTabs(result.mode);
    ...
    return orderedTabs.map(key => ({
      ...
      isPro: proTabs.has(key),
      isLocked: proTabs.has(key) && !isPro,
      preview: result.pro_tab_previews?.[key] || null
    }));
  };

  const tabs = buildTabsFromResult();

  // Debug: Log Pro tabs info (only on result change, not every render)
  const [loggedResultId, setLoggedResultId] = useState(null);
  React.useEffect(() => {
    // Only log when result changes, not on every render
    if (result && result !== loggedResultId && tabs.length > 0) {
      const proTabs = tabs.filter(t => t.isLocked);
      if (proTabs.length > 0) {
        console.log('🔒 Pro Tabs Locked:', proTabs.map(t => `${t.label} (isLocked: true)`));
      }
      setLoggedResultId(result);
    }
  }, [result, tabs.length]);
  ...
  const uploadAttachmentDataUrlToStorage = async (dataUrl, fileNameBase = 'generation-input.jpg') => {
    if (!session?.user || !dataUrl) return null;
    try {
  ...
  return (
    ...
    >
      ...
      <Sidebar
        ...
        usageInfo={{ ...usageInfo, balance: creditInfo.balance, lastReset: creditInfo.lastReset }}
      />
      ...
      <main className="dashboard-main" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', flex: 1, position: 'relative', overflow: 'hidden', marginLeft: isSidebarOpen ? '280px' : '0', transition: 'margin-left 0.3s var(--ease-premium)' }}>
          ...
          {(!currentSession?.id && !result && !loading && !expanderVisible) ? (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', width: '100%', height: '100%' }}>
              ...
              <div className="desktop-only-input animate-fade-in" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
                <div style={{ textAlign: 'center', position: 'relative', marginTop: '-12vh' }}>
                  <h1 style={{ fontSize: '72px', fontWeight: '900', lineHeight: '1', letterSpacing: '-3px', fontStyle: 'italic', margin: 0 }}>
              ...
              {/* ━━━ MOBILE SPLASH (Complete Redesign) ━━━ */}
              <div className="mobile-only-input animate-fade-in" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%', padding: '0 20px', paddingBottom: '8px' }}>

                <Mascot />

                {/* Centered Hero Text */}
                <div style={{ textAlign: 'center', width: '100%', marginBottom: '32px' }}>
                  <h1 style={{ fontSize: '32px', fontWeight: '800', lineHeight: '1.2', letterSpacing: '-1px', margin: 0, color: '#fff' }}>
                    How can I help<br />
                    <span style={{ color: '#a3e635' }}>you</span> today?
                  </h1>
                  <p style={{ color: '#666', marginTop: '12px', fontSize: '14px', fontWeight: '500' }}>
                    AI briefs in 30 seconds
                  </p>
                </div>

                {/* Suggestion Chips — compact horizontal row */}
                <div className="suggestion-chips-row" style={{ display: 'flex', gap: '8px', width: '100%', marginBottom: '16px', justifyContent: 'center' }}>
                  {[
                    ...
                    <button
                      key={idx}
                      onClick={() => chip.text && setSuggestionInput(chip.text)}
                      style={{
                        padding: chip.text ? '10px 16px' : '10px 14px',
                    ...
                    >
                      <span style={{ fontSize: '14px' }}>{chip.icon}</span> {chip.text}
                    </button>
                  ))}
                </div>
              </div>

              {/* DESKTOP CHIPS (kept separate for desktop) */}
              <div className="desktop-only-input" style={{ width: '100%', maxWidth: '800px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' }}>
                <div className="animate-fade-in" style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'center' }}>
                  {[
                    { icon: '💡', text: 'SaaS for students' },
                    { icon: '🚀', text: 'AI tutor app' },
                    { icon: '💻', text: 'Build a portfolio' }
                ...
                <BottomInput onGenerate={handleGenerate} loading={loading} isCentered={true} isSidebarOpen={isSidebarOpen} externalInput={suggestionInput} mode={bottomInputMode} setMode={setBottomInputMode} isPro={usageInfo.isPro} />
              </div>

              {/* MOBILE INPUT (separate so it sticks to bottom) */}
              <div className="mobile-only-input" style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <BottomInput onGenerate={handleGenerate} loading={loading} isCentered={true} isSidebarOpen={isSidebarOpen} externalInput={suggestionInput} mode={bottomInputMode} setMode={setBottomInputMode} isPro={usageInfo.isPro} />
                <MobileCreditWidget credits={creditInfo} isPro={usageInfo.isPro} />
              </div>
            </div>
          ) : expanderVisible ? (
            /* ─── Feature C: Smart Idea Expander (Questions UI) ───────────── */
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', width: '100%', backgroundColor: '#050505', position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 200, overflowY: 'auto' }} className="animate-fade-in">
              {(() => {
                ...
                return (
                  <div style={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%' }}>
                    ...
                    {/* Top Header */}
                    <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', padding: '20px 24px', position: 'relative', zIndex: 10 }}>
                      <button
                        onClick={() => {
                    ...
                    {/* Robot Hero Section */}
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: '10px', marginBottom: '30px', position: 'relative' }}>
                    ...
                    <div style={{ flex: 1, width: '100%', background: '#0a0a0a', borderTopLeftRadius: '32px', borderTopRightRadius: '32px', border: '1px solid rgba(255,255,255,0.05)', borderBottom: 'none', padding: '32px 24px', display: 'flex', flexDirection: 'column' }}>

                      <h2 style={{ fontSize: '26px', fontWeight: '800', color: '#fff', marginBottom: '12px', lineHeight: '1.3', letterSpacing: '-0.5px' }}>
                      ...
                      <p style={{ color: '#888', fontSize: '14px', fontWeight: '500', marginBottom: '32px' }}>Choose the option that best fits your idea.</p>

                      {/* Options */}
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', flex: 1 }}>
                        {(q.type === 'select' || q.type === 'multi_select') && options.length > 0 ? (
                          options.map((o, i) => {
                            const selected = isMulti ? (Array.isArray(current) && current.includes(o.label)) : current === o.label;
                            return (
                              <button key={o.id} onClick={() => toggleOption(o.id, o.label)} style={{
                                width: '100%', padding: '16px', borderRadius: '20px', border: selected ? '1.5px solid #a3e635' : '1px solid rgba(255,255,255,0.05)', background: selected ? 'rgba(163,230,53,0.05)' : '#111', display: 'flex', alignItems: 'center', gap: '16px', cursor: 'pointer', transition: 'all 0.2s', boxShadow: selected ? '0 0 20px rgba(163,230,53,0.1)' : 'none'
                              }}>
                                <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: selected ? 'rgba(163,230,53,0.1)' : 'rgba(255,255,255,0.03)', border: selected ? '1px solid #a3e635' : '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: selected ? '#a3e635' : '#888', fontSize: '16px', fontWeight: '800' }}>
                                  {i + 1}
                                </div>
                                <span style={{ color: selected ? '#fff' : '#ccc', fontSize: '15px', fontWeight: '700', flex: 1, textAlign: 'left' }}>{o.label}</span>
                                {selected && (
                                  <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: '#a3e635', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          ...
                          <input
                            ...
                            style={{ width: '100%', padding: '18px 24px', backgroundColor: '#111', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '20px', color: '#fff', fontSize: '16px', outline: 'none' }}
                          />
                        )}

                        {/* Feedback pill */}
                        {current && (
                          <div className="animate-slide-up" style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px 20px', background: '#111', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.05)', marginTop: '16px' }}>
                            <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#a3e635', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px' }}>🙂</div>
                            <div style={{ flex: 1, fontSize: '13px', color: '#888', lineHeight: '1.4' }}>
                              <span style={{ color: '#a3e635', fontWeight: '700' }}>Nice choice!</span> This will help us craft better and more relevant outputs.
                            </div>
                            <div style={{ fontSize: '18px', color: '#a3e635' }}>✨</div>
                          </div>
                        )}
                      </div>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '32px', marginBottom: '32px' }}>
                        {questions.map((_, i) => (
                          <div key={i} style={{ width: '8px', height: '8px', borderRadius: '50%', background: i === idx ? '#a3e635' : '#333' }}></div>
                        ))}
                      </div>
                    </div>

                    {/* Bottom Action Bar */}
                    <div style={{ display: 'flex', gap: '12px', width: '100%', padding: '16px 24px', background: '#050505', borderTop: '1px solid rgba(255,255,255,0.05)', paddingBottom: 'calc(16px + env(safe-area-inset-bottom))' }}>
                      <button onClick={() => setExpanderStep((s) => Math.max(0, s - 1))} disabled={idx === 0} style={{ padding: '16px', background: '#111', borderRadius: '99px', border: 'none', color: '#fff', fontSize: '14px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px', flex: 1, justifyContent: 'center', opacity: idx === 0 ? 0.5 : 1, cursor: idx === 0 ? 'not-allowed' : 'pointer' }}>
                        <ArrowLeft size={16} /> Back
                      </button>
                      <button
                        onClick={() => {
                          setExpanderAnswers({ _submitted: true });
                          setExpanderVisible(false);
                          handleGenerate(lastInput, selectedMode, { ...(lastGenerationParamsRef.current?.metadata || {}), _skipExpander: true });
                        }}
                        style={{ padding: '16px', background: '#111', borderRadius: '99px', border: 'none', color: '#fff', fontSize: '14px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px', flex: 1, justifyContent: 'center', cursor: 'pointer' }}
                      >
                      ...
                      <button
                        ...
                        disabled={!current && !isLastQuestion} // Prevent moving forward without answering unless skipping
                        style={{ padding: '16px', background: 'linear-gradient(90deg, #84cc16, #a3e635)', borderRadius: '99px', border: 'none', color: '#000', fontSize: '16px', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '8px', flex: 2, justifyContent: 'center', cursor: 'pointer', boxShadow: '0 0 20px rgba(163,230,53,0.3)', opacity: (!current && !isLastQuestion) ? 0.5 : 1 }}
                      >
            ...
            <div style={{ maxWidth: '1000px', margin: '0 auto', width: '100%', paddingBottom: '240px' }}>
              <div className="mobile-score-card" style={{ marginBottom: '24px', padding: '18px 20px', backgroundColor: '#0f0f0f', border: '1px solid #222', borderRadius: '12px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px' }}>
                ...
                {tabs.map(tab => {
                  ...
                  return (
                    <div key={tab.id} style={{ position: 'relative', flexShrink: 0 }}>
                      <button
                        className={tab.isLocked ? 'pro-tab-button' : ''}
                        onClick={() => {
                          if (tab.isLocked) {
                            showToast('👑 Upgrade to Pro to unlock this exclusive tab');
                            setShowUpgradeModal(true);
                          } else {
                            setActiveTab(tab.id);
                          }
                      ...
                      >
                          ...
                          <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', overflow: 'hidden' }}>
                            ...
                            {[...Array(4)].map((_, i) => (
                              <div
                                ...
                                style={{
                                  position: 'absolute',
                                  width: Math.random() * 3 + 2.5 + 'px',
                                  height: Math.random() * 3 + 2.5 + 'px',
                            ...
                            {[...Array(6)].map((_, i) => (
                              <div
                                ...
                                style={{
                                  ...
                                  animation: `float-particle ${Math.random() * 8 + 6}s linear infinite`,
                        ...
                        {tab.isLocked ? (
                          <>
                            <Crown size={14} strokeWidth={2.5} className="pro-crown-icon" style={{ color: "#d97706", position: 'relative', zIndex: 2 }} />
                      ...
                      {/* Pro preview on hover for locked tabs */}
                      {tab.isLocked && tab.preview && (
                        <div style={{
                          position: 'absolute',
                          bottom: '-100px',
                          left: 0,
                        ...
                        >
                          <div style={{ color: '#a3e635', fontSize: '11px', fontWeight: '700', marginBottom: '4px' }}>PRO PREVIEW</div>
                          {tab.preview}
                        </div>
                      )}
                    </div>
                  ...
                  <div key={activeTab} className="animate-fade-in">
                    {activeTab === 'validate' ? (
                        ...
                        {redditInsights && (
                          <div style={{ display: 'grid', gap: '16px' }}>
                            {redditInsights.map((insight, idx) => {
                              const isBlurred = idx > 0 && !usageInfo.isPro;
                              return (
                                <div key={idx} style={{
                                  padding: '24px',
                                  backgroundColor: '#111',
                                  ...
                                  {isBlurred && (
                                    <div style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(13,13,13,0.8)', backdropFilter: 'blur(8px)', zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', textAlign: 'center', padding: '20px' }}>
                                      <Lock size={20} color="#a3e635" style={{ marginBottom: '8px' }} />
                                      <div style={{ fontSize: '11px', fontWeight: '900', color: '#fff', textTransform: 'uppercase', letterSpacing: '1px' }}>Pro Feature</div>
                                      <p style={{ fontSize: '12px', color: '#555', marginTop: '4px' }}>Upgrade to unlock all validation insights.</p>
                                    </div>
                                  )}
                      ...
                      <>
                          ...
                          <div style={{ display: 'flex', gap: '8px' }}>
                            ...
                            {activeTab === 'architecture' && (
                              <button className="action-btn" onClick={handleCursorExport} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', color: '#a3e635', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '1px', padding: '8px 16px', backgroundColor: 'rgba(163, 230, 53, 0.05)', border: '1px solid rgba(163, 230, 53, 0.2)', borderRadius: '8px' }}>
                                <Download size={14} /> <span className="mobile-hide">Export for Cursor</span>
                              </button>
                            )}
                        ...
                        {/* Rendered Markdown Content - Show button for all tabs, content only after loaded */}
                        {/* Check if tab is locked (Pro only) */}
                        {tabs.find(t => t.id === activeTab)?.isLocked ? (
                          <div style={{ textAlign: 'center', padding: '80px 40px', backgroundColor: '#1a0f1a', borderRadius: '16px', border: '1px dashed rgba(163, 230, 53, 0.2)', position: 'relative' }}>
                            <Lock size={48} color="#a3e635" style={{ marginBottom: '24px', opacity: 0.7 }} />
                            <h3 style={{ fontSize: '18px', fontWeight: '900', color: '#a3e635', marginBottom: '8px' }}>Pro Feature 💎</h3>
                            <p style={{ color: '#888', fontSize: '14px', marginBottom: '16px' }}>This tab is exclusive to Pro plan members.</p>
                            ...
                            <button
                              onClick={() => {
                                showToast('🚀 Upgrading to Pro unlocks all exclusive tabs');
                                setShowUpgradeModal(true);
                              }}
                              style={{ padding: '14px 32px', backgroundColor: '#a3e635', color: '#000', fontWeight: '900', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}
                            >
                              ⭐ Upgrade to Pro – $7/month
                            </button>
                            <p style={{ color: '#555', fontSize: '11px', marginTop: '16px' }}>
                              Get instant access to all {tabs.filter(t => t.isPro).length} Pro tabs across all modes.
                            </p>
                          </div>
                          ...
                          <div style={{ textAlign: 'center', padding: '80px 40px', backgroundColor: '#111', borderRadius: '16px', border: '1px dashed #1a1a1a' }}>
                            ...
                            >
                              {loading ? '⏳ Generating...' : '✨ Generate Now (5 credits)'}
                            </button>
                          </div>
                        ) : (
                          <div
        ...
        {(currentSession?.id || result || loading) && !expanderVisible && (
          <div style={{ position: 'relative' }}>
            {/* Usage Meter (for non-pro users) */}
            {!usageInfo.isPro && usageInfo.used > 0 && (
              <div style={{ position: 'fixed', bottom: '100px', right: '32px', padding: '8px 16px', backgroundColor: '#111', border: `1px solid ${usageInfo.used >= 8 ? 'rgba(239, 68, 68, 0.3)' : '#222'}`, borderRadius: '10px', fontSize: '11px', fontWeight: '700', color: usageInfo.used >= 8 ? '#ef4444' : '#888', zIndex: 50 }}>
                {usageInfo.used}/{usageInfo.limit} used today
              </div>
            )}
            <BottomInput
      ...
      {/* ─── CREDITS DEPLETED OVERLAY - ROBOT ATTACKED ──────────────────────────── */}
      {creditInfo?.balance <= 0 && (
        <div style={{
          ...
          <style>{`
            @keyframes stonefallLeft {
              0% { transform: translateY(-150px) translateX(0) rotate(0deg) scale(1); opacity: 1; }
              50% { transform: translateY(50px) translateX(20px) rotate(180deg) scale(1.1); opacity: 1; }
              100% { transform: translateY(220px) translateX(-10px) rotate(360deg) scale(0.7); opacity: 0; }
            }
            @keyframes stonefallCenter {
              0% { transform: translateY(-150px) rotate(0deg) scale(1); opacity: 1; }
              50% { transform: translateY(70px) rotate(180deg) scale(1.15); opacity: 1; }
              100% { transform: translateY(250px) rotate(480deg) scale(0.6); opacity: 0; }
            }
            @keyframes stonefallRight {
              0% { transform: translateY(-150px) translateX(0) rotate(0deg) scale(1); opacity: 1; }
              50% { transform: translateY(60px) translateX(-30px) rotate(180deg) scale(1.1); opacity: 1; }
              100% { transform: translateY(220px) translateX(15px) rotate(360deg) scale(0.7); opacity: 0; }
            }
            @keyframes impactGlow {
              0% { boxShadow: 0 0 5px rgba(163, 230, 53, 0.3); }
              50% { boxShadow: 0 0 25px rgba(163, 230, 53, 1), 0 0 40px rgba(255, 100, 100, 0.6); }
              100% { boxShadow: 0 0 5px rgba(163, 230, 53, 0.3); }
            }
            @keyframes robotShakeSevere {
              0%, 100% { transform: translateX(0) scale(1); filter: brightness(1); }
              5% { transform: translateX(-8px) scale(0.98); filter: brightness(0.95); }
              10% { transform: translateX(8px) scale(1.02); filter: brightness(1.05); }
              15% { transform: translateX(-6px) scale(0.99); filter: brightness(0.97); }
              20% { transform: translateX(6px) scale(1.01); filter: brightness(1.03); }
              25% { transform: translateX(-4px) scale(1); filter: brightness(1); }
              30% { transform: translateX(4px) scale(1); filter: brightness(1); }
              35% { transform: translateX(-2px) scale(1); filter: brightness(1); }
              40%,100% { transform: translateX(0) scale(1); filter: brightness(1); }
            }
            @keyframes robotPulseGlow {
              ...
              50% { 
                textShadow: 0 0 40px rgba(163, 230, 53, 1), 0 0 80px rgba(255, 100, 100, 0.5);
                filter: drop-shadow(0 0 30px rgba(163, 230, 53, 1));
              }
            }
            @keyframes containerPulse {
              0%, 100% { 
                boxShadow: 0 0 60px rgba(163, 230, 53, 0.4), inset 0 0 40px rgba(163, 230, 53, 0.1), 0 0 100px rgba(255, 0, 0, 0.05);
              }
              50% { 
                boxShadow: 0 0 80px rgba(163, 230, 53, 0.7), inset 0 0 60px rgba(163, 230, 53, 0.2), 0 0 150px rgba(255, 0, 0, 0.2);
              }
            }
            @keyframes titleGlitch {
              0%, 100% { color: #a3e635; textShadow: 0 0 20px rgba(163, 230, 53, 0.6); }
              25% { color: #b8f635; textShadow: 0 0 30px rgba(163, 230, 53, 0.8), 2px 2px 10px rgba(255, 100, 100, 0.4); }
              50% { color: #a3e635; textShadow: 0 0 20px rgba(163, 230, 53, 0.6); }
              75% { color: #ff6b6b; textShadow: 0 0 20px rgba(255, 107, 107, 0.6), -2px -2px 10px rgba(163, 230, 53, 0.4); }
            }
            @keyframes borderPulse {
              0%, 100% { borderColor: #a3e635; }
              50% { borderColor: #b8f635; boxShadow: 0 0 60px rgba(163, 230, 53, 0.4), inset 0 0 40px rgba(163, 230, 53, 0.1), 0 0 100px rgba(255, 0, 0, 0.2); }
            }
            ...
            @keyframes buttonPulse {
              0%, 100% { boxShadow: 0 0 30px rgba(163, 230, 53, 0.4), 0 8px 0 rgba(0, 0, 0, 0.2); }
              50% { boxShadow: 0 0 50px rgba(163, 230, 53, 0.6), 0 8px 0 rgba(0, 0, 0, 0.2); }
            }
            @keyframes shimmer {
            ...
            .stone-left { animation: stonefallLeft 2.5s cubic-bezier(0.25, 0.46, 0.45, 0.94) infinite; }
            .stone-center { animation: stonefallCenter 2.2s cubic-bezier(0.25, 0.46, 0.45, 0.94) infinite; }
            .stone-right { animation: stonefallRight 2.4s cubic-bezier(0.25, 0.46, 0.45, 0.94) infinite; }
            .robot-container { animation: robotShakeSevere 3s ease-in-out infinite, robotPulseGlow 2.5s ease-in-out infinite; willChange: 'transform, filter'; backfaceVisibility: 'hidden'; }
            .container { animation: containerPulse 2.8s ease-in-out infinite, borderPulse 2.8s ease-in-out infinite; }
            .title { animation: titleGlitch 4s ease-in-out infinite; willChange: 'color, text-shadow'; }
            .grid-bg { animation: gridFlicker 3s ease-in-out infinite; }
            .btn-primary { animation: buttonBreathe 3s ease-in-out infinite, buttonPulse 3s ease-in-out infinite; }
          `}</style>

          {/* Animated Stones - Multiple patterns */}
          {[...Array(3)].map((_, i) => (
            <div key={`left-${i}`} className="stone stone-left" style={{
              left: `${15 + i * 5}%`,
              top: `-20px`,
              ...
              filter: 'drop-shadow(0 0 8px rgba(163, 230, 53, 0.7))'
            }} />
          ))}

          {[...Array(3)].map((_, i) => (
            <div key={`center-${i}`} className="stone stone-center" style={{
              left: `${45 + i * 5}%`,
              top: `-30px`,
              animationDelay: `${i * 0.35}s`,
              width: '16px',
              height: '16px',
              backgroundColor: '#b8f635',
              borderRadius: '50%',
              boxShadow: '0 0 14px rgba(184, 246, 53, 1)',
              filter: 'drop-shadow(0 0 10px rgba(184, 246, 53, 0.8))'
            }} />
          ))}

          {[...Array(3)].map((_, i) => (
            <div key={`right-${i}`} className="stone stone-right" style={{
              left: `${70 + i * 5}%`,
              top: `-25px`,
              ...
              backgroundColor: '#a3e635',
              borderRadius: '50%',
              boxShadow: '0 0 11px rgba(163, 230, 53, 0.95)',
              filter: 'drop-shadow(0 0 7px rgba(163, 230, 53, 0.75))'
            }} />
          ))}
          ...
          <div className="container" style={{
            ...
            textAlign: 'center',
            padding: '48px 40px',
            background: 'linear-gradient(135deg, #0d0d0d 0%, #0a0a0a 50%, #1a0a1a 100%)',
            borderRadius: '32px',
            ...
            <div style={{ position: 'relative', zIndex: 10 }}>
              ...
              <div style={{
                ...
                Your prompt engine has exhausted its energy reserves.
                <br />Each generation draws 10-15 credits from your core.
              </div>
              ...
              <div style={{
                ...
                <div style={{ borderRight: '1px solid rgba(163, 230, 53, 0.1)' }}>
                  <div style={{
                    fontSize: '48px',
                    fontWeight: '900',
                    color: '#ff5555',
                    textShadow: '0 0 20px rgba(255, 85, 85, 0.6)'
                  }}>0</div>
                  <div style={{ fontSize: '11px', color: '#a3e635', fontWeight: '700', textTransform: 'uppercase', marginTop: '8px', letterSpacing: '1px' }}>Power Available</div>
                </div>
                <div>
                  <div style={{
                    fontSize: '48px',
                    fontWeight: '900',
                    color: '#a3e635',
                    textShadow: '0 0 20px rgba(163, 230, 53, 0.6)'
                  }}>10</div>
                  <div style={{ fontSize: '11px', color: '#a3e635', fontWeight: '700', textTransform: 'uppercase', marginTop: '8px', letterSpacing: '1px' }}>Energy Cost</div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '12px', flexDirection: 'column' }}>
                <button
                  ...
                  style={{
                    ...
                    boxShadow: '0 0 30px rgba(163, 230, 53, 0.4), 0 8px 0 rgba(0, 0, 0, 0.2)',
                    textTransform: 'uppercase',
                    letterSpacing: '1.5px',
                    position: 'relative',
                    overflow: 'hidden',
                  ...
                  onMouseEnter={(e) => {
                    ...
                    e.target.style.boxShadow = '0 0 60px rgba(163, 230, 53, 0.9), 0 0 30px rgba(184, 246, 53, 0.6), inset 0 0 20px rgba(255, 255, 255, 0.3), 0 4px 0 rgba(0, 0, 0, 0.2)';
                    e.target.style.transform = 'scale(1.05) translateY(-4px)';
                  }}
                  onMouseLeave={(e) => {
                    e.target.style.backgroundColor = '#a3e635';
                    e.target.style.boxShadow = '0 0 30px rgba(163, 230, 53, 0.4), 0 8px 0 rgba(0, 0, 0, 0.2)';
                    e.target.style.transform = 'scale(1) translateY(0)';
                  }}
                  onMouseDown={(e) => {
                    e.target.style.transform = 'scale(0.98) translateY(4px)';
                    e.target.style.boxShadow = '0 0 20px rgba(163, 230, 53, 0.5), inset 0 4px 10px rgba(0, 0, 0, 0.3)';
                  }}
                  onMouseUp={(e) => {
                    e.target.style.transform = 'scale(1.05) translateY(-4px)';
                    e.target.style.boxShadow = '0 0 60px rgba(163, 230, 53, 0.9), 0 0 30px rgba(184, 246, 53, 0.6), inset 0 0 20px rgba(255, 255, 255, 0.3), 0 4px 0 rgba(0, 0, 0, 0.2)';
                ...
                <button
                  ...
                  style={{
                    ...
                    letterSpacing: '1px',
                    position: 'relative',
                    willChange: 'border-color, color, box-shadow'
                  }}
                  onMouseEnter={(e) => {
                    e.target.style.borderColor = '#a3e635';
                    e.target.style.color = '#a3e635';
                    e.target.style.boxShadow = '0 0 30px rgba(163, 230, 53, 0.4), inset 0 0 20px rgba(163, 230, 53, 0.05)';
                    e.target.style.transform = 'translateY(-2px)';
                  }}
                  onMouseLeave={(e) => {
                    e.target.style.borderColor = 'rgba(163, 230, 53, 0.3)';
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <span>Daily reset at midnight. Credits expire after 30 days of inactivity.</span>
                </div>

                {/* Countdown Timer - Matching TopBar style */}
                <div style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '10px',
                  ...
                  <div>
                    <div style={{ fontSize: '10px', fontWeight: '800', color: '#555', textTransform: 'uppercase', marginBottom: '4px' }}>NEXT RESET IN</div>
                    <div style={{
                      fontSize: '14px',
                      fontWeight: '800',
                      color: '#a3e635',
                      ...
                      {String(resetCountdown.hours).padStart(2, '0')}:{String(resetCountdown.minutes).padStart(2, '0')}:{String(resetCountdown.seconds).padStart(2, '0')}
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* ─── Upgrade Modal (Hidden for Pro Users) ──────────────────────────── */}
      {showUpgradeModal && !usageInfo.isPro && (
        <div onClick={() => setShowUpgradeModal(false)} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div onClick={e => e.stopPropagation()} className="animate-fade-in" style={{ width: '100%', maxWidth: '480px', padding: '48px', backgroundColor: '#0d0d0d', border: '1px solid #6d28d9', borderRadius: '24px', textAlign: 'center' }}>
            <div style={{ fontSize: '48px', marginBottom: '16px' }}>🚀</div>
            <h2 style={{ fontSize: '24px', fontWeight: '900', marginBottom: '8px' }}>Out of Credits!</h2>
            <p style={{ color: '#888', fontSize: '14px', marginBottom: '32px' }}>Upgrade to Pro to boost your daily credit limit, gain unlimited community access, and unlock powerful export tools.</p>
            <div style={{ padding: '16px', backgroundColor: '#111', borderRadius: '12px', border: '1px solid #222', marginBottom: '24px' }}>
              <a href="/pricing" style={{ display: 'inline-block', padding: '14px 24px', backgroundColor: '#a3e635', color: '#000', borderRadius: '8px', fontWeight: '700', textDecoration: 'none' }}>Upgrade to Pro →</a>
            </div>
            <button onClick={() => setShowUpgradeModal(false)} style={{ color: '#555', fontSize: '13px', fontWeight: '700', background: 'none', border: 'none', cursor: 'pointer' }}>Maybe later</button>
          </div>
        </div>
      )}
          ...
          <div id="share-card-wrapper" onClick={e => e.stopPropagation()} style={{ position: 'relative' }}>
            ...
            <div id="share-card-content" style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '80px' }}>
                ...
                <span style={{ fontSize: '18px', fontWeight: '900', letterSpacing: '-0.5px' }}>Prompt<span style={{ color: '#a3e635' }}>OS</span></span>
              ...
              <div style={{ marginTop: 'auto', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                ...
                {/* Free Watermark - More Prominent */}
                {(!usageInfo?.isPro && !profile?.is_pro) && (
                  <div style={{
                    fontSize: '11px',
      ...
      {/* ─── Upgrade Modal (Hidden for Pro Users) ───────────────────────────── */}
      {showUpgradeModal && !usageInfo.isPro && (
        <div className="upgrade-modal-overlay" style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.9)', zIndex: 1200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          <div className="animate-fade-in upgrade-modal-container" style={{ width: '100%', maxWidth: '440px', backgroundColor: '#0a0a0a', border: '1px solid #333', borderRadius: '24px', padding: '40px', position: 'relative' }}>
            <button onClick={() => setShowUpgradeModal(false)} style={{ position: 'absolute', top: '16px', right: '16px', background: 'none', border: 'none', color: '#666', cursor: 'pointer' }}><X size={20} /></button>
            <div style={{ textAlign: 'center', marginBottom: '32px' }}>
              <div style={{ width: '64px', height: '64px', backgroundColor: 'rgba(163, 230, 53, 0.1)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                <Zap size={32} color="#a3e635" />
              </div>
              <h2 style={{ fontSize: '24px', fontWeight: '900', color: '#fff', marginBottom: '8px' }}>Unlock Limitless Generation</h2>
              <p style={{ color: '#888', fontSize: '14px', lineHeight: '1.5' }}>You have hit your limit of {usageInfo.limit} prompts. Upgrade to Pro for infinite ideas & full validation tools.</p>
            </div>

            <div style={{ backgroundColor: '#111', borderRadius: '16px', border: '1px solid #222', padding: '24px', marginBottom: '24px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {['Unlimited Generations', 'All Expert AI Modes', 'Export Code to IDE', 'Deep Reddit Validation'].map((feat, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ width: '16px', height: '16px', backgroundColor: 'rgba(163,230,53,0.1)', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Check size={10} color="#a3e635" /></div>
                    <span style={{ fontSize: '13px', color: '#ccc', fontWeight: '600' }}>{feat}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ textAlign: 'center', marginTop: '16px' }}>
              <button onClick={() => { setShowUpgradeModal(false); navigate('/pricing'); }} style={{ background: 'none', border: 'none', color: '#888', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}>Compare standard plans →</button>
            </div>
          </div>
        </div>
      )}
