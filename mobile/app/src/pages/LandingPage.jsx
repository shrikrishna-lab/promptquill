...
import { getMobileRenderProfile, isNativeMobile } from '../lib/platform.mobile';
...
const LandingPage = ({ session }) => {
    ...
    // Auto-open AuthModal if referral code is present
    useEffect(() => {
    ...
    const onAuthClick = () => session ? navigate('/ai') : setAuthOpen(true);
    ...
    const handlePricingCheckout = async (type) => {
        ...
        try {
            ...
            const map = {
                credits_49: 'CREDITS_49',
                credits_99: 'CREDITS_99',
                credits_249: 'CREDITS_249'
            };
            const pkg = map[type];
            if (!pkg) {
    ...
    useEffect(() => {
        ...
        const setupVisuals = async () => {
            const isMobileViewport = window.innerWidth <= 768;
            const nativeWebView = isNativeMobile();
            const renderProfile = getMobileRenderProfile();
            const lenis = nativeWebView ? null : new Lenis({ duration: 1.2, easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t)), smooth: true, direction: 'vertical', gestureDirection: 'vertical' });
            let lenisTicker = null;

            if (lenis && !isMobileViewport) {
            ...
            scene.fog = new THREE.FogExp2(0x000000, nativeWebView ? 0.018 : 0.012);
            const camera = new THREE.PerspectiveCamera(60, window.innerWidth / window.innerHeight, 0.1, 1000);
            camera.position.z = nativeWebView ? 28 : 25;
            ...
            renderer.setPixelRatio(renderProfile.maxPixelRatio);

            const engineGroup = new THREE.Group();
            scene.add(engineGroup);

            const geoOuter = new THREE.IcosahedronGeometry(10, nativeWebView ? 1 : 2);
            const matOuter = new THREE.MeshBasicMaterial({ color: 0xA8FF3E, wireframe: true, transparent: true, opacity: nativeWebView ? 0.1 : 0.15 });
            ...
            const matInner = new THREE.MeshBasicMaterial({ color: 0x7B2FFF, wireframe: true, transparent: true, opacity: nativeWebView ? 0.25 : 0.4 });
            const meshInner = new THREE.Mesh(geoInner, matInner);
            engineGroup.add(meshInner);

            const dustGeo = new THREE.BufferGeometry();
            const dustPos = new Float32Array(renderProfile.particles * 3);
            for (let i = 0; i < renderProfile.particles * 3; i++) dustPos[i] = (Math.random() - 0.5) * 80;
            dustGeo.setAttribute('position', new THREE.BufferAttribute(dustPos, 3));
            const dustMat = new THREE.PointsMaterial({ size: nativeWebView ? 0.08 : 0.1, color: 0xffffff, transparent: true, opacity: nativeWebView ? 0.2 : 0.3 });
            ...
            const handleResize = () => {
                ...
                renderer.setSize(window.innerWidth, window.innerHeight);
                renderer.setPixelRatio(getMobileRenderProfile().maxPixelRatio);
            };
            window.addEventListener('resize', handleResize);

            const ctx = gsap.context(() => {
                if (isMobileViewport || nativeWebView) return;
            ...
            let animId;
            const animate = () => {
                dust.rotation.y += nativeWebView ? 0.0005 : 0.001;
                meshOuter.rotation.x += nativeWebView ? 0.001 : 0.002;
                meshInner.rotation.y -= nativeWebView ? 0.0015 : 0.003;
            ...
            const refreshTimer = window.setTimeout(() => {
                if (!nativeWebView) ScrollTrigger.refresh();
        ...
        setupVisuals().catch((error) => {
            console.error('Landing visuals failed to initialize:', error);
        });
        ...
        <div className="main-container" ref={containerRef}>
            ...
            <section className="pricing-section" id="pricing" style={{ padding: '120px 5%', background: '#020202', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ textAlign: 'center', marginTop: '70px', position: 'relative', zIndex: 2 }}>
                    <p style={{ color: '#444', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '2px' }}>Secure military-grade payments via <strong style={{ color: '#777' }}>Secure Payment</strong></p>
                </div>
            </section>
            ...
            <footer className="dashboard-footer" style={{ borderTop: '1px solid #1a1a1a', padding: '80px 5% 40px', background: '#000', position: 'relative', zIndex: 10 }}>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '40px', maxWidth: '1200px', margin: '0 auto', marginBottom: '60px' }}>
                    <div style={{ flex: '2 1 350px', opacity: 0.9 }}>
                        ...
                        <p style={{ color: '#666', fontSize: '14px', lineHeight: '1.6', maxWidth: '280px' }}>AI prompt engineering engine with 6 generation modes, Bot & Human voice toggle, and multi-tab briefs. 10 free generations daily. Secure payments.</p>
                    </div>
