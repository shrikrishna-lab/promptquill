...
const Sidebar = ({ user, profile, sessions, currentSessionId, onNewSession, onSessionSelect, onVersionSelect, onRename, onDelete, isOpen, onToggle, usageInfo, onSettings, onTogglePublic, isPublic }) => {
  ...
  const clearHistory = async () => {
    if (window.confirm('Are you sure you want to clear all history?')) {
      ...
      if (error) console.error('Error clearing history:', error.message);
      ...
      >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '4px' }}>
              <div style={{ fontSize: '18px', fontWeight: '800', color: '#fff', display: 'flex', alignItems: 'center', gap: '4px', letterSpacing: '-0.5px', lineHeight: 1 }}>
                PromptQuill <span style={{ fontSize: '14px' }}>🌿</span>
              </div>
        ...
        <div onClick={() => { profile?.username && navigate(`/u/${profile.username}`); onToggle(); }} style={{ background: '#0d0d0d', border: '1px solid #1a1a1a', borderRadius: '16px', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <div style={{ color: '#fff', fontSize: '14px', fontWeight: '700', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '140px' }}>{profile?.username || user?.email?.split('@')[0] || 'User'}</div>
              <div style={{ color: '#666', fontSize: '12px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '140px' }}>{user?.email || 'krishna@example.com'}</div>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', paddingBottom: '80px' }}>
          ...
          <div>
            ...
            <div style={{ background: '#0d0d0d', border: '1px solid #1a1a1a', borderRadius: '16px', overflow: 'hidden' }}>
              {[
                { icon: <Palette size={18} />, label: 'Creative Gallery', to: '/gallery' },
          ...
          <div>
            ...
            <div style={{ background: '#0d0d0d', border: '1px solid #1a1a1a', borderRadius: '16px', overflow: 'hidden' }}>
              {[
                ...
                { icon: <Award size={18} />, label: 'Score Card', to: '/scorecard' }
              ].map((item, i) => (
          ...
          {/* Special Cards */}
          <Link to="/wrapped" onClick={onToggle} style={{ background: '#0d0d0d', border: '1px solid #1a1a1a', borderRadius: '16px', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', textDecoration: 'none' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <Zap size={20} color="#facc15" />
              <div>
                <div style={{ color: '#fff', fontSize: '13px', fontWeight: '700' }}>2025 Wrapped</div>
                <div style={{ color: '#666', fontSize: '10px' }}>See your year in prompts</div>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <span style={{ background: 'rgba(168,85,247,0.15)', color: '#c084fc', padding: '2px 8px', borderRadius: '99px', fontSize: '9px', fontWeight: '800' }}>New</span>
              <ChevronRight size={16} color="#444" />
            </div>
          </Link>

          <div style={{ background: '#0d0d0d', border: '1px solid #1a1a1a', borderRadius: '16px', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <Star size={20} color="#a3e635" />
              <div>
                <div style={{ color: '#fff', fontSize: '13px', fontWeight: '700' }}>Love PromptQuill?</div>
                <div style={{ color: '#666', fontSize: '10px' }}>Invite friends & earn credits</div>
              </div>
            </div>
            <button onClick={() => { navigate('/referral'); onToggle(); }} style={{ background: '#a3e635', border: 'none', borderRadius: '10px', padding: '6px 12px', color: '#000', fontSize: '11px', fontWeight: '800', cursor: 'pointer' }}>
              Invite
            </button>
          </div>

          {isAdmin && (
            <Link to="/admin" onClick={onToggle} style={{ width: '100%', background: '#0d0d0d', border: '1px solid #1a1a1a', borderRadius: '16px', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', textDecoration: 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#a3e635' }}>
                <Shield size={18} />
                <span style={{ fontSize: '13px', fontWeight: '700' }}>Admin Panel</span>
              </div>
              <ChevronRight size={16} color="#444" />
            </Link>
          )}
          ...
          <div style={{ display: 'flex', gap: '12px', width: '100%' }}>
            ...
            <button onClick={() => supabase.auth.signOut()} style={{ flex: 1, background: 'rgba(239, 68, 68, 0.05)', border: '1px solid rgba(239, 68, 68, 0.2)', borderRadius: '16px', padding: '14px 16px', display: 'flex', justifyContent: 'center', alignItems: 'center', cursor: 'pointer' }}>
        ...
        <div style={{ padding: '24px', flex: 1, overflowY: 'auto' }} className="no-scrollbar">
          ...
          <div style={{ marginBottom: '32px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <Link to="/community" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', borderRadius: '8px', color: '#888', textDecoration: 'none', fontSize: '13px', fontWeight: '600' }}>
                <Globe size={18} /> Community Feed
              </Link>
              <Link to="/gallery" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', borderRadius: '8px', color: '#888', textDecoration: 'none', fontSize: '13px', fontWeight: '600' }}>
                <Palette size={18} style={{ color: '#db2777' }} /> Creative Gallery
              </Link>
              ...
              <div style={{ paddingTop: '12px', borderTop: '1px solid rgba(163, 230, 53, 0.1)' }}>
                ...
                <Link to="/scorecard" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', borderRadius: '8px', color: '#888', textDecoration: 'none', fontSize: '13px', fontWeight: '600' }}>
                  <Award size={18} /> Score Card
                </Link>
              </div>

              <Link to="/support" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', borderRadius: '8px', color: '#888', textDecoration: 'none', fontSize: '13px', fontWeight: '600' }}>
              ...
              <Link to="/wrapped" className="animate-shimmer" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', borderRadius: '8px', color: '#fff', textDecoration: 'none', fontSize: '13px', fontWeight: '800', marginTop: '8px' }}>
                <Zap size={18} color="#facc15" /> 2025 Wrapped
              </Link>
              <button onClick={() => navigate('/referral')} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', borderRadius: '8px', color: '#fff', textDecoration: 'none', fontSize: '13px', fontWeight: '800', background: 'linear-gradient(135deg, #a3e635, #6d28d9)', border: 'none', cursor: 'pointer', width: '100%', justifyContent: 'flex-start' }}>
                <Globe size={18} /> Refer to earn
              </button>
            </div>
          </div>
        ...
        <div style={{ padding: '20px', backgroundColor: '#0a0a0a', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button style={{ color: '#ef4444', padding: '4px' }} title="Log out" onClick={() => supabase.auth.signOut()}>
                <LogOut size={16} />
              </button>
            </div>
          </div>
          ...
          {isAdmin && (
            <Link to="/admin" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', fontSize: '11px', color: '#a3e635', fontWeight: '700', marginTop: '16px' }}>
