...
const BottomInput = ({ onGenerate, loading, isCentered, isSidebarOpen, externalInput = '', mode, setMode, isPro = false }) => {
  ...
  const [showImageCreditConfirm, setShowImageCreditConfirm] = useState(false);
  ...
  const allModes = [
    ...
    { id: 'STARTUP', label: ' Startup PRO', icon: '🚀', color: '#f97316', isPro: true, credits: 25 },
    { id: 'STARTUP_LITE', label: 'Startup Lite', icon: '✨', color: '#fbbf24', credits: 10 },
  ...
  const submitWithCurrentState = () => {
    ...
    if (attachment) {
      ...
      metadata.imageCreditRange = '7-10';
    }
  ...
  const handleSubmit = () => {
    if (attachment && !showImageCreditConfirm) {
      setShowImageCreditConfirm(true);
      return;
    }
  ...
  return (
    <div className="bottom-input-container" style={
      ...
      <div style={{ width: '100%', maxWidth: '800px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '16px', alignItems: 'center' }}>
        ...
        <div className="desktop-only-input" style={{ width: '100%' }}>
          <div className="input-pill-wrapper" style={{
            ...
            {attachment && (
              <span style={{ marginLeft: '8px', fontSize: '11px', fontWeight: 800, color: '#fbbf24', border: '1px solid rgba(251,191,36,0.4)', borderRadius: '999px', padding: '5px 8px', whiteSpace: 'nowrap' }}>
                7-10 credits
              </span>
            )}
          </div>
        ...
        <div className="mobile-only-input" style={{
          ...
          {attachment && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '4px 8px', borderRadius: '12px', border: '1px solid #2a2a2a', background: '#0b0b0b', width: 'max-content', marginBottom: '8px' }}>
              ...
              <span style={{ fontSize: '10px', color: '#fbbf24' }}>7-10 credits</span>
            </div>
          )}
      ...
      {showImageCreditConfirm && (
        ...
        >
          <div style={{ width: '92%', maxWidth: '460px', background: '#0f0f0f', border: '1px solid #2a2a2a', borderRadius: '16px', padding: '20px' }}>
            <h3 style={{ margin: 0, marginBottom: '8px', color: '#fff', fontSize: '16px' }}>Image Analysis Credit Confirmation</h3>
            <p style={{ margin: 0, color: '#b3b3b3', fontSize: '13px' }}>
              Analyzing this image will cost 7–10 credits. Continue?
            </p>
            <div style={{ display: 'flex', gap: '10px', marginTop: '16px' }}>
              <button
                onClick={() => setShowImageCreditConfirm(false)}
                style={{ flex: 1, padding: '10px', borderRadius: '10px', border: '1px solid #333', background: '#111', color: '#aaa', fontWeight: 700, cursor: 'pointer' }}
              >
                Cancel
              </button>
              <button
                onClick={() => {
                  setShowImageCreditConfirm(false);
                  submitWithCurrentState();
                }}
                style={{ flex: 1, padding: '10px', borderRadius: '10px', border: 'none', background: '#a3e635', color: '#000', fontWeight: 800, cursor: 'pointer' }}
              >
                Confirm
              </button>
            </div>
