...
const stringifyNestedValue = (value) => {
  if (value === null || value === undefined) return '';
  if (Array.isArray(value)) return value.map(stringifyNestedValue).filter(Boolean).join(', ');
  if (typeof value === 'object') {
    return Object.entries(value)
      .map(([key, nestedValue]) => `${humanizeKey(key)}: ${stringifyNestedValue(nestedValue)}`)
...
export const formatTabValue = (value, tabKey = '') => {
  if (value === null || value === undefined) return '';
  ...
  if (typeof value === 'string') {
    ...
    if ((text.startsWith('[') && text.endsWith(']')) || (text.startsWith('{') && text.endsWith('}'))) {
      try {
        return formatTabValue(JSON.parse(text), tabKey);
      } catch {
        return value;
      }
    }
  ...
  if (Array.isArray(value)) {
    return value.map((item, index) => {
      ...
      const title = item.step || item.action || item.title || item.name || `${humanizeKey(tabKey) || 'Item'} ${index + 1}`;
      const titleText = String(title).replace(/^\d+\.?\s*/, '').trim();
      const lines = [`${index + 1}. **${titleText}**`];

      Object.entries(item).forEach(([key, nestedValue]) => {
        if (['step', 'action', 'title', 'name'].includes(key)) return;
        const formatted = stringifyNestedValue(nestedValue);
        if (!formatted) return;
  ...
  if (typeof value === 'object') {
    const xmlKeys = ['role', 'context', 'task', 'constraints', 'output_format'];
    if (xmlKeys.some((key) => Object.prototype.hasOwnProperty.call(value, key))) {
      return xmlKeys
        .filter((key) => value[key] !== undefined && value[key] !== null)
        .map((key) => `<${key}>${stringifyNestedValue(value[key])}</${key}>`)
        .join('\n');
    }

    return Object.entries(value)
      .map(([key, nestedValue]) => {
        const formatted = formatTabValue(nestedValue, key);
        if (!formatted) return '';
...
export const validateAndMergeBrief = (parsed, fallbackMode, mode = 'GENERAL') => {
  ...
  if (acceptLegacyTabs && hasV4Tabs && !hasV5Tabs) {
    ...
    const receivedTabCount = Object.keys(workingBrief.tabs || {}).filter(k => {
      const val = toTabString(workingBrief.tabs[k]);
    ...
    for (const tabKey of actualTabsToProcess) {
      let tabValue = toTabString(workingBrief.tabs?.[tabKey]);
  ...
  const merged = {
    ...
    quality_rating: workingBrief.quality_rating && typeof workingBrief.quality_rating === 'object'
      ? workingBrief.quality_rating
      : {
          role_clarity: score_breakdown.clarity * 10,
          context_depth: score_breakdown.specificity * 10,
          task_precision: score_breakdown.clarity * 10,
          constraint_spec: score_breakdown.feasibility * 10,
          output_format: score_breakdown.specificity * 10,
          overall: score * 10,
          stars: Math.max(1, Math.min(5, Math.round(score / 2))),
          label: score >= 8 ? 'Strong' : score >= 6 ? 'Solid' : 'Needs Work',
          weakest_element: workingBrief.score_reasoning || fallback.score_reasoning
        },
    tabs
  };
