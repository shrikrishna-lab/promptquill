...
const GenerationForm = ({ onGenerate, loading, onCreditsInsufficient, credits: propsCredits }) => {
  ...
  const loadCredits = async () => {
    try {
      const userCredits = await getCredits(DEMO_USER_ID);
      console.log('✓ Credits loaded:', userCredits);
      setCredits(userCredits);
    } catch (err) {
      console.error('Error loading credits:', err);
      setCredits({ balance: 999, lastReset: new Date().toISOString() });
    }
  };

  const loadReferralCode = async () => {
    const code = await getReferralCode(DEMO_USER_ID);
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '24px', display: 'flex', gap: '12px', flexDirection: 'column' }}>
              <div style={{ color: '#888', textAlign: 'center', padding: '20px', fontSize: '14px' }}>Open source — payment handled by your own setup</div>
              <button
