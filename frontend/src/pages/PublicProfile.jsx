...
import { ArrowLeft, GitBranch, ExternalLink, Globe, Zap, ChevronUp, Award, Share2, Copy, UserPlus } from 'lucide-react';
...
const PublicProfile = () => {
  ...
  const [following, setFollowing] = useState(() => {
    try { return JSON.parse(localStorage.getItem('promptquill_following_profiles') || '[]').includes(username); } catch { return false; }
  });
  ...
  const toggleFollow = () => {
    let list = [];
    try { list = JSON.parse(localStorage.getItem('promptquill_following_profiles') || '[]'); } catch { list = []; }
    const next = following ? list.filter(u => u !== username) : [...new Set([...list, username])];
    localStorage.setItem('promptquill_following_profiles', JSON.stringify(next));
      ...
      <div className="animate-fade-in" style={{ maxWidth: '800px', margin: '0 auto', padding: '48px 24px' }}>
        <style>{`
          @media (max-width: 680px) {
            .public-profile-header { flex-direction: column; align-items: stretch !important; }
            .public-profile-links { flex-direction: column !important; gap: 10px !important; }
            .public-profile-stats { grid-template-columns: repeat(2, 1fr) !important; }
            .public-profile-actions { width: 100%; justify-content: stretch !important; }
            .public-profile-actions button { flex: 1; }
          }
        `}</style>
        <div className="public-profile-header" style={{ display: 'flex', alignItems: 'flex-start', gap: '24px', marginBottom: '40px' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div className="public-profile-actions" style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap' }}>
              <button onClick={toggleFollow} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '10px 14px', borderRadius: '12px', border: '1px solid #a3e635', background: following ? 'rgba(163,230,53,0.12)' : 'transparent', color: '#a3e635', fontWeight: 800, cursor: 'pointer' }}><UserPlus size={16} /> {following ? 'Following' : 'Follow'}</button>
              <button onClick={copyProfileLink} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '10px 14px', borderRadius: '12px', border: '1px solid #222', background: '#111', color: '#ddd', fontWeight: 800, cursor: 'pointer' }}><Copy size={16} /> {copied ? 'Copied' : 'Copy Link'}</button>
              <button onClick={() => navigator.share?.({ title: `@${profile.username}`, url: window.location.href }) || copyProfileLink()} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '10px 14px', borderRadius: '12px', border: '1px solid #222', background: '#111', color: '#ddd', fontWeight: 800, cursor: 'pointer' }}><Share2 size={16} /> Share</button>
            </div>
            <div className="public-profile-links" style={{ display: 'flex', gap: '16px' }}>
        ...
        {/* Stats */}
        <div className="public-profile-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '48px' }}>
          {[
